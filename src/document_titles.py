"""
Why this file? So an answer can cite "DGUV Information 203-071 - Wiederkehrende
Prüfungen" instead of "203-071.pdf".

Titles come from the documents themselves, not from a hand-written list: the point of
the system is that any DGUV PDF dropped into data/ works. Metadata first, cover page as
a fallback, filename as the last resort - it never returns nothing.

Not part of the index. Titles are display text, and storing them on every chunk would
mean re-indexing whenever one changed.
...
QUOTES = re.compile(r'[„“”"»«]')
MIN_USABLE_TITLE = 12 # Shorter than this and the metadata is treated as empty

# Cover lines that are never part of a title. Anything not listed here can still end up
# in a fallback title - a wordy title beats a missing one.
...
class TitleReader:
    ...
    def read(self, path: Path) -> str:
        """The best available title for one PDF, never empty."""
        doc = fitz.open(path)
        try:
            title = self._from_metadata(doc)
            if len(QUOTES.sub("", title).strip()) < MIN_USABLE_TITLE:
                title = self._from_cover(doc)
        finally:
            doc.close()
        return self._tidy(title) or path.stem
    ...
    def _from_cover(self, doc) -> str:
        """Assemble a title from the first page: the designation plus a few real lines."""
        ...
        body = [
            ...
            if line != designation and len(line) > MIN_USABLE_TITLE and not COVER_NOISE.search(line)
        ]
        return " ".join(([designation] if designation else []) + body[:3])

    @staticmethod
    def _undouble(text: str) -> str:
        """Drop a title the PDF producer stored twice in a row."""
        half = len(text) // 2
    ...
    def _tidy(text: str) -> str:
        ...
        mid-phrase, leaving an opening quote that reads like a corrupted name.
        """
...
def load_titles(path: Path | None = None) -> dict[str, str]:
    """
    Read the title map ----> rag_engine

    An empty map is not an error: every caller falls back to the filename, so the system
    works on a corpus this has never been run against.
    """
